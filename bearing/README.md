...
1. [프로젝트 개요](#프로젝트-개요)
2. [이 데모와 실제 도입의 차이](#이-데모와-실제-도입의-차이)
3. [서비스 구성도](#서비스-구성도)
4. [주요 기능](#주요-기능)
5. [모델 및 프롬프트](#모델-및-프롬프트)
6. [기대효과 및 개선점](#기대효과-및-개선점)
7. [실행 방법](#실행-방법)
...
**지금 보시는 화면은 실제 사내 시스템에 연결된 게 아니라, 더미(가상) 데이터로 파이프라인의 판단 로직만 시연하는 프로토타입입니다.** "감지 → 검증 → 합의 → 실행 → 통보 → 학습"이라는 의사결정 구조와 LLM 프롬프트 설계가 이 프로젝트의 핵심이고, 그 앞뒤로 붙는 데이터 입출력은 실제 도입 시 아래처럼 사내 시스템으로 교체되는 것을 가정합니다.

| 구분 | 지금 데모 | 실제 현업 도입 시 |
|---|---|---|
| 화물 데이터 | `data/shipments.csv` 고정 50건 | 사내 TMS/ERP(화물관리시스템) API로 실시간 조회 |
| 노선/운임 데이터 | `data/routes.csv` 고정 30건 | 실시간 운임 지수(Freightos, Xeneta 등) + 사내 계약운임 연동 |
| 뉴스 감지 | 캐시 뉴스 20건 또는 해운 전문지 RSS 4곳 | (RSS는 이미 부분 구현됨) 유료 해운 정보 서비스(Lloyd's List, Alphaliner 등) 구독 API까지 확장 |
| 사건 감지 방식 | 사용자가 "뉴스 스캔 실행" 버튼을 눌러야 동작 | 백그라운드 상시 모니터링 파이프라인이 사건을 자동 감지해 알림·큐잉 |
| 과거 사건 이력 | `state.py`에 하드코딩된 13건 | 사내 위기대응 이력 DB(과거 실제 파업/기상/통관 이슈 로그) 연동 |
| 반대심문·합의 페르소나 | 데모 시나리오(현대글로비스 가상 조직)에 맞춘 시스템 프롬프트 | 실제 조직 구조·팀명·결재 권한에 맞게 프롬프트 커스터마이징, 실제 결재 시스템과 연동 |
| 고객 메일 | 3종 생성 후 다운로드만 가능 | 사내 CRM/메일 시스템과 연동해 승인 후 즉시 발송 |
| 플레이북 문서 | `.md` 파일 다운로드 | 사내 협업툴(Notion, Confluence, Slack 등)에 자동 게시·버전 관리 |
| 세션/이력 저장 | `session_state`만 사용, 새로고침 시 초기화 | DB 도입으로 사건별 이력이 영구 보존되고 여러 담당자가 이어서 작업 가능 |

즉, **"뉴스에서 리스크를 읽고 판단을 검증하고 합의를 도출하는" AI 추론 로직 자체는 이 데모와 실제 운영 환경에서 동일하게 재사용**되고, 달라지는 부분은 그 앞뒤로 붙는 데이터 파이프라인(어디서 데이터를 가져오고 어디로 결과를 보내는지)입니다. 자세한 한계점은 [기대효과 및 개선점 — 한계점](#한계점) 항목을 참고하세요.
